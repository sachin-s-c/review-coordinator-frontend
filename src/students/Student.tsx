...
import { getStudentData } from "../redux /studentSlice"
...
const getSearch=(value:string)=>{
  ...
  let filter
 filter = studentData.filter(reviewer=>{return  reviewer.name.toLowerCase().includes(value.toLowerCase())})
...
useEffect(()=>{
  ...
  dispatch(getStudentData(students))
